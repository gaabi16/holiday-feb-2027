...
/*LAND-DATA-END*/
  ...
  function Globe(el) {
    ...
    var spans = (route.spans || []).map(function (s) {
      ...
      return { text: s.text, mid: at(m), prev: at(m - 3), next: at(m + 3) };
    });
    ...
    /* cât de departe e un punct de cel mai apropiat aeroport vizibil */
    function clearance(x, y, wpPos) {
      var best = 1e9, i, p, d;
      for (i = 0; i < wpPos.length; i++) {
        p = wpPos[i];
        if (!p) continue;
    ...
    function pill(lines, x, y, muted) {
      ...
      ctx.strokeStyle = muted ? "rgba(139,166,189,.5)" : "rgba(255,176,32,.45)";
      ctx.lineWidth = 1;
      if (muted) ctx.setLineDash([3, 2]);
      ctx.beginPath();
      if (ctx.roundRect) ctx.roundRect(bx, by, bw, bh, 6);
      else ctx.rect(bx, by, bw, bh);
      ctx.fill();
      ctx.stroke();
      if (muted) ctx.setLineDash([]);
      ctx.fillStyle = muted ? "#8ba6bd" : "#ffb020";
    ...
    function draw() {
      ...
      /* pozițiile aeroporturilor pe ecran, ca etichetele să se ferească de ele */
      var wpPos = [];
